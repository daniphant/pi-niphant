...
describe("integration", () => {
  it("loads extension without browser dependencies", async () => { const tools:any[]=[]; ext({ registerTool:(t:any)=>tools.push(t) } as any); expect(tools.length).toBe(2); expect(JSON.stringify(tools)).not.toMatch(/agent-browser|playwright|screenshot/i); });
